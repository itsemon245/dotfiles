...
| 2.5.9 | Move Hyprland startup orchestration to `tools/.local/bin/hypr-startup` backed by `dotfiles_tools.hypr`, then update `hyprland.lua` to call it directly and remove `startup/launch.sh`. | High | Dry-run or mocked startup confirms intended commands, log file paths, dependency skips, and no duplicate service starts. |
